...
async def stop_deployment(
    ...
    try:
        deployment = await deployment_service.stop_deployment(
            deployment_id,
            force=force
        )
        return DeploymentResponse(**deployment)
...
async def restore_backup(
    ...
    try:
        deployment = await deployment_service.restore_backup(
            deployment_id,
            snapshot_name
        )
        return DeploymentResponse(**deployment)
...
@router.get(
    ...
    description="Stream deployment logs (WebSocket endpoint)"
)
async def get_deployment_logs(
    ...
    TODO: Implement log streaming from VM
    """
    raise HTTPException(
...
@router.get(
    ...
    description="Check if deployments service is healthy"
)
