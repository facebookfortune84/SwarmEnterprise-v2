...
SHEETS_ENDPOINT = os.getenv("SHEETS_ENDPOINT")  # Prefer an Apps Script webhook URL or internal sync endpoint
...
def push_row(payload: dict) -> Optional[dict]:
    ...
    try:
        import requests
        r = requests.post(SHEETS_ENDPOINT, json=payload, timeout=int(os.getenv('CONNECTOR_TIMEOUT', 10)))
        r.raise_for_status()
        return r.json()
