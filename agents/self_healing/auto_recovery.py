...
class AutoRecoveryAgent:
    ...
    async def recover_component(self, component: str) -> RecoveryReport:
        ...
        component_health = next(
            (c for c in health_report.components if c.component == component), None
        )
    ...
    async def _execute_action(
        self, component: str, action: RecoveryAction, strategy: RecoveryStrategy
    ) -> RecoveryAttempt:
    ...
    async def _execute_builtin_action(
        self, component: str, action: RecoveryAction
    ...
    async def _restart_service(self, component: str) -> tuple[RecoveryStatus, Optional[str]]:
        ...
        try:
            ...
            result = subprocess.run(
                ["systemctl", "restart", component], capture_output=True, text=True, timeout=30
            )
    ...
    async def _restart_container(self, component: str) -> tuple[RecoveryStatus, Optional[str]]:
        ...
        try:
            ...
            result = subprocess.run(
                ["docker", "restart", component], capture_output=True, text=True, timeout=60
            )
    ...
    async def _escalate_recovery(
        self, component: str, strategy: RecoveryStrategy
    ) -> RecoveryReport:
    ...
    def _determine_final_status(
        self, attempts: List[RecoveryAttempt], health_after: HealthStatus
    ) -> RecoveryStatus:
    ...
    def _register_default_strategies(self):
        ...
        self.register_strategy(
            RecoveryStrategy(
                ...
                actions=[RecoveryAction.RESTART_CONTAINER, RecoveryAction.CLEAR_CACHE],
                max_attempts=3,
                cooldown_seconds=60,
                escalation_actions=[RecoveryAction.RESTART_SERVICE, RecoveryAction.ROLLBACK],
            )
        )
        ...
        self.register_strategy(
            RecoveryStrategy(
                ...
                actions=[RecoveryAction.FLUSH_CONNECTIONS, RecoveryAction.RESTART_CONTAINER],
                max_attempts=2,
                cooldown_seconds=120,
                escalation_actions=[RecoveryAction.FAILOVER],
            )
        )
