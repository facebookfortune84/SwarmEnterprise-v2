        ...
        if self.state == CircuitState.OPEN:
            ...
            if self.opened_at:
                ...
                if elapsed >= self.config.timeout_seconds:
                    ...
                    self._change_state(CircuitState.HALF_OPEN, "Timeout elapsed, testing recovery")
                    self.half_open_calls = 0
                    return True
            return False
    ...
    async def _record_call(self, success: bool, duration: float, error: Optional[Exception]):
        ...
        if success:
            ...
            self.recent_calls.append(
                {"success": True, "duration": duration, "timestamp": datetime.utcnow()}
            )
            ...
            if self.state == CircuitState.CLOSED:
                ...
                if (
                    ...
                    or self._calculate_recent_error_rate() >= self.config.error_rate_threshold
                ):
                    self._change_state(
                        CircuitState.OPEN,
                ...
                self._change_state(CircuitState.OPEN, "Service still failing in half-open state")
                self.opened_at = datetime.utcnow()
...
What is the likely cause and recommended action?"""
            ...
            insight = await self.ollama.generate(prompt=prompt, temperature=0.3)
...
def circuit_breaker(
    name: str, manager: CircuitBreakerManager, config: Optional[CircuitConfig] = None
):
    """
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        circuit = manager.create_circuit(
            "api_service", CircuitConfig(failure_threshold=3, timeout_seconds=30)
        )
