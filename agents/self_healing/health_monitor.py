...
class HealthMonitor:
    ...
    def register_health_check(
        self, component: str, component_type: ComponentType, check_func: Callable
    ):
        """
        ...
        """
        self.health_checks[component] = {"type": component_type, "func": check_func}
    ...
    async def check_health(self) -> HealthReport:
        ...
        avg_response_time = (
            sum(c.response_time_ms for c in components) / len(components) if components else 0
        )
        ...
        recommendations = await self._generate_recommendations(
            components, resource_metrics, error_rate, avg_response_time
        )
    ...
    async def _run_health_check(self, component: str, config: Dict) -> HealthCheck:
    ...
    def _calculate_overall_status(
        self, components: List[HealthCheck], resources: ResourceMetrics
    ) -> HealthStatus:
        ...
        if (
            resources.cpu_percent >= self.thresholds.cpu_critical
            or resources.memory_percent >= self.thresholds.memory_critical
            or resources.disk_percent >= self.thresholds.disk_critical
        ):
            return HealthStatus.CRITICAL
        ...
        if (
            degraded_components
            or resources.cpu_percent >= self.thresholds.cpu_warning
            or resources.memory_percent >= self.thresholds.memory_warning
            or resources.disk_percent >= self.thresholds.disk_warning
        ):
...
What is the most likely root cause and immediate action?"""
        ...
        try:
            response = await self.ollama.generate(prompt=prompt, temperature=0.3)
    ...
    def get_component_history(self, component: str, limit: int = 100) -> List[HealthCheck]:
