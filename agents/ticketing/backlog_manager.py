...
from agents.ticketing.ticket_prioritizer import PriorityScore, PriorityLevel, TicketAnalysis
...
class BacklogManager:
    ...
    def __init__(self, ollama_client: Optional[OllamaClient] = None, model: str = "llama3"):
    ...
    async def analyze_backlog(
        self, tickets: List[TicketAnalysis], priority_scores: List[PriorityScore]
    ) -> BacklogReport:
        """
        ...
        epics, duplicates, stale, blocked, sprints = await asyncio.gather(
            epics_task, duplicates_task, stale_task, blocked_task, sprints_task
        )
        ...
        health = self._assess_health(
            len(tickets), priority_dist, len(duplicates), len(stale), len(blocked)
        )

        # Generate recommendations
        recommendations = await self._generate_recommendations(
            health, len(tickets), priority_dist, len(duplicates), len(stale), len(blocked)
        )
    ...
    async def find_duplicates(self, tickets: List[TicketAnalysis]) -> List[DuplicateGroup]:
        """
    ...
    async def find_stale_tickets(self, tickets: List[TicketAnalysis]) -> List[str]:
        """
        ...
        stale = [t.ticket_id for t in tickets if t.updated_at < threshold]
    ...
    async def _group_by_theme(self, tickets: List[TicketAnalysis]) -> Dict[str, List[str]]:
        """Group tickets by theme/feature area"""
    ...
    async def _generate_epic_title(self, theme: str, tickets: List[TicketAnalysis]) -> str:
        """Generate epic title from theme and tickets"""

        # Simple title generation
        return f"{theme.title()} Epic"

    async def _generate_epic_description(self, theme: str, tickets: List[TicketAnalysis]) -> str:
...
Focus on the overall goal and value."""
        ...
        try:
            description = await self.ollama.generate(prompt=prompt, temperature=0.5)
    ...
    def _estimate_epic_effort(
        self, ticket_ids: List[str], score_map: Dict[str, PriorityScore]
    ) -> str:
    ...
    async def _calculate_similarity(
        self, ticket1: TicketAnalysis, ticket2: TicketAnalysis
    ) -> float:
    ...
    async def _explain_duplication(
        self, primary: TicketAnalysis, duplicates: List[TicketAnalysis]
    ) -> str:
    ...
    def _identify_focus_areas(self, scores: List[PriorityScore]) -> List[str]:
    ...
    def _identify_sprint_risks(self, scores: List[PriorityScore]) -> List[str]:
