...
class OracleAssets:
    ...
    def _load_all(self):
        ...
        logger.info(
            f"✅ Assets loaded: {len(self.prompts)} prompts, {len(self.sops)} SOPs, {len(self.tools)} tools"
        )
    ...
    def get_sops_for_role(self, role: str) -> List[Dict[str, str]]:
        ...
        for sop_name, sop_content in self.sops.items():
            ...
            if role_lower in sop_name_lower or role_lower in sop_content[:500].lower():
                relevant.append(
                    {
                        "name": sop_name,
                        "content": (
                            sop_content[:2000] + "..." if len(sop_content) > 2000 else sop_content
                        ),
                    }
                )
    ...
    def get_tools_for_role(self, role: str) -> List[Dict[str, Any]]:
        ...
        for tool_name, tool_config in self.tools.items():
            ...
            if role_lower in tool_name_lower or any(cat in tool_name_lower for cat in categories):
                relevant.append(
                    tool_config
                    if isinstance(tool_config, dict)
                    else {"name": tool_name, "config": tool_config}
                )
