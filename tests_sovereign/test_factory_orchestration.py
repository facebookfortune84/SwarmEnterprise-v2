...
from backend.services.company_generator import CompanyGenerator, CompanyRequest, TechStack, GenerationStatus
...
async def test_full_factory_pipeline_orchestration(db):
    ...
    with patch("agents.managers.board.strategic_board.convene", return_value=mock_tickets), \
         patch("agents.workers.executor_critic.execution_unit.process_ticket", return_value=mock_worker_result):
...
async def test_factory_failure_handling(db):
    ...
    with patch("agents.managers.board.strategic_board.convene", side_effect=Exception("LLM Timeout")):
        result = await generator.generate_company(request)
