...
def assert_api_error(response: Dict[str, Any], expected_status: int, expected_message: Optional[str] = None):
...
def assert_valid_company(company: Dict[str, Any]):
    ...
    assert company["status"] in valid_statuses, \
        f"Invalid status: {company['status']}"
    
    # Validate tech_stack
    valid_stacks = ["fastapi-react-postgres", "nodejs-tailwind-mongo", "django-vue-mysql"]
    assert company["tech_stack"] in valid_stacks, \
        f"Invalid tech_stack: {company['tech_stack']}"
...
def assert_valid_deployment(deployment: Dict[str, Any]):
    ...
    assert deployment["status"] in valid_statuses, \
        f"Invalid status: {deployment['status']}"
    
    # Validate URL format
    assert deployment["url"].startswith("https://"), \
        f"Invalid URL format: {deployment['url']}"
...
def assert_valid_user(user: Dict[str, Any]):
    ...
    assert user["subscription_tier"] in valid_tiers, \
...
def assert_valid_ticket(ticket: Dict[str, Any]):
    ...
    assert ticket["status"] in valid_statuses, \
        f"Invalid status: {ticket['status']}"
