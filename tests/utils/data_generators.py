...
def generate_companies(
    count: int = 10, user_ids: Optional[List[str]] = None
) -> List[Dict[str, Any]]:
...
def generate_deployments(
    count: int = 10, company_ids: Optional[List[str]] = None
) -> List[Dict[str, Any]]:
...
def generate_api_keys(count: int = 5) -> List[str]:
    ...
    return [
        f"sk_test_{random_string(32, string.ascii_letters + string.digits)}" for _ in range(count)
    ]
