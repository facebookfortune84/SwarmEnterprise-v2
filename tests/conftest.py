...
def mock_stripe():
    ...
    with patch("stripe.Customer") as mock_customer, patch(
        "stripe.Subscription"
    ) as mock_subscription, patch("stripe.checkout.Session") as mock_session:
...
def mock_ollama():
    ...
    with patch("requests.get") as mock_get, patch("requests.post") as mock_post:
        ...
        mock_post.return_value = Mock(
            status_code=200, json=lambda: {"message": MOCK_LLM_RESPONSES["code_generation"]}
        )
